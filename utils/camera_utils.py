...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    if cam_info.objects is not None:
        # from pngimagefile to tensor
        objects = torch.from_numpy(np.array(cam_info.objects)).float().unsqueeze(0)
        resized_objects = torchvision.transforms.Resize(
            resolution, interpolation=InterpolationMode.NEAREST)(objects)
    ...
    if cam_info.cx is not None and cam_info.cy is not None:
        ...
        scale_fx /= scale
        scale_fy /= scale
    if cam_info.depths is not None:
