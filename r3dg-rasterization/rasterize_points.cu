	...
	const bool debug)
{
  ...
  return std::make_tuple(rendered, n_contrib, out_color, out_objects, out_opacity, out_depth, out_feature, out_normal, out_surface_xyz, radii, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const bool debug) 
{
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dobjects, dL_dopacity, dL_dmeans3D, dL_dfeatures, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations);
}
