...
def render_view(viewpoint_camera: Camera, pc: GaussianModel, pipe, bg_color: torch.Tensor,
                scaling_modifier=1.0, override_color=None, is_training=False, dict_params=None, mask=None):
    ...
    if pipe.compute_neilf_python:
        ...
        brdf_color, extra_results = rendering_equation(
            base_color, roughness, metallic, normal.detach(), viewdirs, incidents,
            is_training, direct_light_env_light, visibility, sample_num=dict_params['sample_num'])
    # print(brdf_color.shape, normal.shape, base_color.shape, roughness.shape, metallic.shape)
    if is_training:
    ...
    (num_rendered, num_contrib, rendered_image, rendered_objects, rendered_opacity, rendered_depth,
    ...
    results = {"render": rendered_image,
               ...
               "name": viewpoint_camera.image_name}
...
def calculate_loss(viewpoint_camera, pc, results, opt, mask=None):
    tb_dict = {
    ...
    if mask is None:
        ...
        gt_image = ((viewpoint_camera.depths).float().permute(2,0,1))[3:6].cuda()
        gt_image = gt_image * (~mask.unsqueeze(0).repeat(3,1,1))
        # print(gt_image)
        # print()
        rendered_image = rendered_image  * (~mask.unsqueeze(0).repeat(3,1,1))
        # print(rendered_image)
        # input()   
        Ll1 = F.l1_loss(rendered_image, gt_image)
        ssim_val = ssim(rendered_image, gt_image)
        # print("loss l1:", Ll1)
        # print("loss ssim:", ssim_val)
    tb_dict["l1"] = Ll1.item()
    ...
    if opt.lambda_pbr > 0:
        if mask is None:
            ...
            rendered_pbr = rendered_pbr * (~mask.unsqueeze(0).repeat(3,1,1))
            Ll1_pbr = F.l1_loss(rendered_pbr, gt_image)
            ssim_val_pbr = ssim(rendered_pbr, gt_image)
...
def render_neilf(viewpoint_camera: Camera, pc: GaussianModel, pipe, bg_color: torch.Tensor,
                 ...
                 is_training=False, dict_params=None, mask=None):
    """
    Render the scene.
    ...
    if is_training:
        loss, tb_dict = calculate_loss(viewpoint_camera, pc, results, opt, mask)
