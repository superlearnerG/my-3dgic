...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        if raster_settings.debug:
            ...
            try:
                num_rendered, num_contrib, color, objects, opacity, depth, feature, normal, surface_xyz, radii, geomBuffer, binningBuffer, imgBuffer = _C.rasterize_gaussians(
                    *args)
            ...
            # remove num_contrib because missing
            num_rendered, num_contrib, color, objects, opacity, depth, feature, normal, surface_xyz, radii, geomBuffer, binningBuffer, imgBuffer = _C.rasterize_gaussians(
                *args)
        ...
        ctx.save_for_backward(colors_precomp, means3D, features, scales, rotations, cov3Ds_precomp,
                              radii, sh, sh_objs, geomBuffer, binningBuffer, imgBuffer)
        return num_rendered, num_contrib, color, objects, opacity, depth, feature, normal, surface_xyz, radii

    @staticmethod
    def backward(ctx, grad_num_rendered, grad_num_contrib, grad_out_color, grad_out_objects, grad_out_opacity, grad_out_depth,
                 grad_out_feature, grad_out_normal, grad_out_surface_xyz, grad_out_radii):
        # Restore necessary values from context
        num_rendered = ctx.num_rendered
        raster_settings = ctx.raster_settings
        colors_precomp, means3D, features, scales, rotations, cov3Ds_precomp, radii, sh, sh_objs, \
        ...
        if raster_settings.debug:
            cpu_args = cpu_deep_copy_tuple(args)  # Copy them before they can be corrupted
            try:
                grad_means2D, grad_colors_precomp, grad_objects, grad_opacities, grad_means3D, grad_features, grad_cov3Ds_precomp, \
...
class GaussianRasterizer(nn.Module):
    ...
    def forward(self, means3D, means2D, opacities, shs=None, sh_objs=None, colors_precomp=None,
